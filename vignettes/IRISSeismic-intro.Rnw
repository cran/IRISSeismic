...
\begin{itemize}
  ...
  \item \code{SensitivityFrequency} -- the frequency (Hz) at which the \code{InstrumentSensitivity} is correct
  \item \code{InputUnits} -- instrument data acquisition input units
...
\begin{itemize}
  ...
  \item \code{includerestricted} -- optional flag whether to report on restricted data (default=\code{FALSE}) 
