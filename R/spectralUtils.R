...
crossSpectrum <- function(x, spans = NULL, kernel = NULL, taper = 0.1,
  ...
  DF <- data.frame(
    freq = freq, spec1 = spec1, spec2 = spec2, coh = coh, phase = phase,
...
psdList2NoiseMatrix <- function(psdList, evalresp = NULL) {
  ...
  if (is.null(evalresp)) {
    evalresp <- getEvalresp(iris, network, station, location, channel,
      time = starttime + 1,
  ...
  if (nrow(rawNoiseMatrix) != psdCount) {
    stop(paste(
      "psdList2NoiseMatrix: psdCount =", psdCount,
  ...
  if (ncol(rawNoiseMatrix) != length(evalresp$freq)) {
    stop(paste(
      "psdList2NoiseMatrix: length(evalresp$freq) =", length(evalresp$freq),
...
psdPlot <- function(PSDs, style = "psd", evalresp = NULL, ylo = -200, yhi = -50, showNoiseModel = TRUE,
  ...
  legend("topright",
    bty = "n", inset = c(0.09, 0),
    legend = c(
      paste(starttime, "  start "),
      paste(endtime, "  end ")
    )
  )
}
